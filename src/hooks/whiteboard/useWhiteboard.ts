...
export const useWhiteboard = (roomId: string | undefined, accessToken: string | null, user: any) => {
  ...
  const [size, setSize] = useState<number>(1); // size in PTS
      ...
      if (points.length > 0) {
        socketRef.current.send(JSON.stringify({
          ...
          data: {
            user_id: user?.id,
            points,
            color: tool === 'eraser' ? 'transparent' : '#ee2689',
            lineWidth: size * 20,
  ...
  const startDrawing = (e: any) => {
    ...
    if (tool === 'eraser') contextRef.current!.globalCompositeOperation = 'destination-out';
      ...
      contextRef.current!.strokeStyle = '#ee2689';
    }
    ...
    if (currentWhiteboardId) {
      fetch(`${API_URL}/whiteboard/strokes`, {
        ...
        body: JSON.stringify({
          ...
          data: { points: currentStrokePoints.current, color: tool === 'eraser' ? 'transparent' : '#ee2689', lineWidth: size * 20 }
        })
      });
    }
  };
  ...
  return { canvasRef, tool, setTool, size, setSize, isAdmin, clearCanvas, downloadPNG, startDrawing, draw, finishDrawing };
};
